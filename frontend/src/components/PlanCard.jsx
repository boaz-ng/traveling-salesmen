...
function PlanCard({ plan, expanded, selected, onClick, onAddToTrip, tripSaving, tripFlights }) {
  const {
    rank,
    price,
    score,
  ...
  const alreadyInTrip = Array.isArray(tripFlights) && tripFlights.some(f => {
          ...
          <div className="pt-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div className="flex items-center gap-2">
              {onAddToTrip && (
                <button
                  type="button"
                  className={`inline-flex items-center gap-1.5 rounded-full border px-4 py-2 text-xs font-semibold transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed ${
                    alreadyInTrip
                      ? 'border-[#2F855A]/30 text-[#2F855A] bg-[#F0FFF4]'
                      : 'border-[#D6C6A8]/50 text-[#9C8A6A] hover:bg-[#F7F5EF] hover:border-[#9C8A6A]/40'
                  }`}
                  disabled={alreadyInTrip || tripSaving === plan.id}
                  onClick={(e) => {
                    e.stopPropagation()
                    onAddToTrip(plan)
                  }}
                >
                  ...
                  {alreadyInTrip ? 'Added' : tripSaving === plan.id ? 'Saving...' : 'Add to Trip'}
                </button>
              )}
              <button
                type="button"
